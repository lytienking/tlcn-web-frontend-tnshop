...
class ShopDetail extends Component {
    ...
    addToFavorite() {
        ...
        (async()=>{
            try {
                let response=await userApi.addToFavorite({idShirt:this.state.idShirt});

                if(response.success){
                    this.setState({
                        openAlert:true,
                        contentAlert:"Đã thêm thành công. Chuyển sang Quản lí tài khoản để xem chi tiết",
                        typeAlert:"success"
                    });
                } else{
                    this.setState({
